...
function downloadCheckedLinks() {
    for (var i = 0; i < visibleLinks.length; ++i) {
        if (document.getElementById('check' + i).checked) {
            chrome.downloads.download({ url: visibleLinks[i], filename: allFilenames[i] },
                function (id) {
                });
        }
    }
...
// multiple times.
chrome.extension.onRequest.addListener(function (datalist) {
    for (var index in datalist) {
        allLinks.push(datalist[index]['href']);
        allFilenames.push(datalist[index]['filename']);
    }
    // allLinks.sort();
